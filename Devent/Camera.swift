...
//  Devent
//
//  Created by Erman Sefer on 28/10/15.
...
class Camera {
    ...
    class func shouldStartCamera(target: AnyObject, canEdit: Bool, frontFacing: Bool) -> Bool {
        ...
        if target is MessageViewController {
            cameraUI.delegate = target as! MessageViewController
        }
    ...
    class func shouldStartPhotoLibrary(target: AnyObject, canEdit: Bool) -> Bool {
        ...
        if target is MessageViewController {
            imagePicker.delegate = target as! MessageViewController
        } 
